...
In this lab, you're going to take WordCount (an existing Hadoop application that is extensively described in the [Hadoop tutorial](https://hadoop.apache.org/docs/r3.0.3/hadoop-mapreduce-client/hadoop-mapreduce-client-core/MapReduceTutorial.html)) and modify it into UrlCount. You can either approach the lab as native Java-hadoop application or use the [Hadoop streaming API](https://www.michael-noll.com/tutorials/writing-an-hadoop-mapreduce-program-in-python/) to implement the lab in Python with the Streaming API.
...
+ The [dataproc](https://www.cloudskillsboost.google/focuses/586?parent=catalog) CloudSkillBoost. This will get you familiar with starting a cluster on the Google Cloud Platform.  NOTE: READ THIS WHOLE DOC FIRST.  You'll be able to complete the dataproc tutorial fast enough, where you can then run your experiments needed for this assignments.  You can always go through it a second time (we have plenty of CloudSkillBoost credits).

You should first confirm that you can run the existing WordCount1 program on the [CSEL Coding environment](https://coding.csel.io/) and (later) on the Google `dataproc` environment. Then, create the URLCount tool using the Coding environment -- use that easier (and cheaper) environment to debug your code. Then, lastly, run your URLCount implementation using Google's `dataproc`.

You will need to use `git` throughout this lab. You're going to first develop your code on CSEL Coding environment, check it into Git, then use the Google `dataproc` environment. You'll get your code on that environment by using `git clone` to checkout your repo.  See the [ssh tutorial video](https://canvas.colorado.edu/courses/109065/pages/video-tutorial-on-using-ssh-keys-with-github-and-google-cloud?module_item_id=5713724) if you need guidance on that still.
...
Once you have your code working on Coding, you're ready to spend some dollars on `dataproc`. Spin up a cluster with one master node and two worker nodes.  NOTE: You can use the dataproc tutorial environment to complete this (you might need to go through the tutorial again -- this will save our GCP credits for later).  

Login to the master node and checkout your Git repo on that system. You may need to create additional rules or modify parts of the Makefile to run your application in that environment because they use a different version of Java and Hadoop and there are dependences on the installation paths.  For hadoop, you can run `hadoop version' to see the version.  I don't think you'll need to do anything different with java.
...
You should create a file `SOLUTION.md` that briefly describes your solution and what software is needed for it to run. You should indicate what resources you used and anyone you worked with as described in the course collaboration policy.  You don't need to use markdown, but if you want formatted text (e.g., headings, bold text, etc.), here's a cheatsheet: [https://www.markdownguide.org/cheat-sheet/](https://www.markdownguide.org/cheat-sheet/)
...
Commit your code by the due state with the `SOLUTION.md` file and push it to Github. Make certain you can see your results on the Github website.  The submit the assignment in Canvas with your github repo URL.
